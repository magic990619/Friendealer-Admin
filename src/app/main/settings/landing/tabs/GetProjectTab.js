...
import GetProjectDialog from './GetProjectDialog'
...
class GetProjectTab extends Component {
    ...
    componentDidMount() {
        ...
        .then(res => {
            this.setState({rows: res.data.projects});
        });
    }
    ...
    handleSave = (project, type) => {
        var rows = this.state.rows;
        var res = [];
        if (type === 'edit') {
            api.post('/auth/updateProject', {project});
            rows.forEach(function(cur, err) {
                if (cur._id !== project._id)
                    res.push(cur);
                else res.push(project);
            });
        }
        else {
            api.post('/auth/addProject', {project}).then(res => project._id = res.data.doc._id);
            res = rows;
            res.push(project);
        }
        console.log(res);
    ...
    handleRemove = (row) => {
        ...
        api.post('/auth/removeProjectById', {_id: row._id});
        ...
        return (
            <div>
                <div className="p-24 flex flex-1 flex-col items-center justify-center md:flex-row md:items-center">
                    ...
                    <GetProjectDialog type='add' onSave={this.handleSave} onRemove={this.handleRemove} row={{_id: '', name: '', budget: ''}}/>
                </div>
                        ...
                        {data.map(row => (
                            <TableRow key={row._id}>
                                ...
                                <TableCell align="center">
                                    <GetProjectDialog type='edit' onSave={this.handleSave} onRemove={this.handleRemove} row={row}/>
                                </TableCell>
                            </TableRow>
                        ))}
                        {
...
export default withStyles(styles)(GetProjectTab);