...
import ExploreEventDialog from './ExploreEventDialog'
...
class EventTypeTab extends Component {
    ...
    componentDidMount() {
        ...
        .then(res => {
            this.setState({rows: res.data.explores});
        });
    }
    ...
    handleSave = (explore, type) => {
        var rows = this.state.rows;
        var res = [];
        if (type === 'edit') {
            api.post('/auth/updateExplore', {explore});
            rows.forEach(function(cur, err) {
                if (cur._id !== explore._id)
                    res.push(cur);
                else res.push(explore);
            });
        }
        else {
            api.post('/auth/addExplore', {explore}).then(res => explore._id = res.data.doc._id);
            res = rows;
            res.push(explore);
        }
        console.log(res);
    ...
    handleRemove = (row) => {
        ...
        api.post('/auth/removeExploreById', {_id: row._id});
        ...
        return (
            <div>
                <div className="p-24 flex flex-1 flex-col items-center justify-center md:flex-row md:items-center">
                    ...
                    <ExploreEventDialog type='add' onSave={this.handleSave} onRemove={this.handleRemove} row={{_id: '', name: '', budget: ''}}/>
                </div>
                        ...
                        {data.map(row => (
                            <TableRow key={row._id}>
                                ...
                                <TableCell align="center">
                                    <ExploreEventDialog type='edit' onSave={this.handleSave} onRemove={this.handleRemove} row={row}/>
                                </TableCell>
                            </TableRow>
                        ))}
                        {
